...
describe("AdminProductsService.createProduct", () => {
  it("rejects a variant selecting an option value that wasn't declared in options[]", async () => {
    ...
    await expect(service.createProduct(input)).rejects.toThrow(BadRequestException);
  ...
  it("rejects an unknown tax class code", async () => {
    ...
    await expect(service.createProduct(validInput)).rejects.toThrow(BadRequestException);
  });
  ...
  it("rejects an unknown categoryId", async () => {
    ...
    await expect(
      service.createProduct({ ...validInput, categoryIds: ["nonexistent-category"] }),
