  ...
  if (error) {
    ...
    return (
      <ErrorState
        ...
        description={response.status === 403 ? t("forbiddenDescription") : t("detailErrorDescription")}
      />
    );
  }
    ...
    <div>
      ...
      <div className="mt-4 flex flex-wrap items-center justify-between gap-4">
        ...
        <div className="flex flex-wrap items-center gap-3">
          {canFulfill ? <OrderFulfillmentActions orderId={order.orderId} status={order.status} /> : null}
          {canRefund ? (
      ...
      <div className="mt-8 grid gap-8 lg:grid-cols-[1fr_320px]">
              ...
              <TableBody>
                {order.items.map((item) => (
                  <TableRow key={item.id}>
                    ...
                    <TableCell numeric>{formatMoney(item.lineTotal.amountMinor, locale)}</TableCell>
                  </TableRow>
                ))}
              </TableBody>
            </Table>
                  ...
                  <TableBody>
                    {order.payments.map((payment) => (
                      <TableRow key={payment.id}>
                        ...
                        <TableCell numeric>{formatMoney(payment.amount.amountMinor, locale)}</TableCell>
        ...
        <Card className="h-fit">
          ...
          <div className="flex flex-col gap-2">
            <SummaryRow label={td("subtotal")} value={formatMoney(order.subtotal.amountMinor, locale)} />
            <SummaryRow label={td("shipping")} value={formatMoney(order.shipping.amountMinor, locale)} />
