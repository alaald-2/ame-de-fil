...
// DRAFT -> PUBLISHED -> ARCHIVED only, mirroring the real state machine
// admin-products.service.ts enforces (LEGAL_STATUS_TRANSITIONS) — mirrored
// here only to decide which single action button to show, exactly like
// order-status.ts's canMarkReadyToShip/canMarkShipped/canMarkDelivered. The
// backend's own guarded update is the real enforcement, not this.
