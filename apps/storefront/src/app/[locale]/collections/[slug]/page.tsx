...
import { buildCollectionHref } from "../../../../lib/pagination-href";
...
async function loadCollection(slug: string, locale: AppLocale, page: number) {
  const result = await api.GET("/api/v1/collections/{slug}", {
    params: { path: { slug }, query: { locale, page, pageSize: 24 } },
  });
  return unwrapOrNotFound(result, "Failed to load collection");
}
