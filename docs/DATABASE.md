...
2. A BullMQ delayed job is scheduled for `expiresAt`. On firing, it re-checks the reservation is still `PENDING` (not already consumed by a completed payment) and, if so, releases it in a transaction (decrement `reserved`, mark reservation `EXPIRED`) — **idempotent**: safe to run twice, safe to race against step 3.
3. On a verified `payment_intent.succeeded` webhook (never on browser redirect — see `PAYMENTS.md`), within a transaction: confirm the `StockReservation` is still valid (not expired/consumed), convert it to a permanent decrement (`InventoryMovement` of type `SALE`, `onHand -= qty`, `reserved -= qty`), mark the reservation `CONSUMED`, and transition the `Order`/`Payment` state machines. If the reservation already expired before the webhook arrived (slow customer + fast TTL), the order is **not** silently confirmed — it goes to a `PAYMENT_SUCCEEDED_STOCK_LOST` admin-alert state for manual resolution (refund or backorder), rather than either overselling or silently taking money for nothing.
